...
if __name__ == "__main__":
    ...
    train_log_file = f"results/{args.dataset_name}_n_clusters_{args.n_clusters}_num_K_{args.num_k}_minADE_Loss_{args.minADEloss}_training_log.csv"
    with open(train_log_file, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['epoch', 'dataset', 'total_loss', 'ade', 'fde', 'min_ade', 'min_fde', 'num_traj', 'min_fde_epoch'])

    for ep in range(args.epoch):

        total_loss = train(ep, model, reg_criterion, cls_criterion, optimizer, train_loader, 
                           motion_modes,args.just_x_y, args.num_k, args.ped_num_k,args.minADEloss)
        ...
        with open(train_log_file, 'a', newline='') as f:
            writer = csv.writer(f)
            writer.writerow([ep, args.dataset_name, train_loss, ade, fde, min_ade, min_fde, num_traj, min_fde_epoch])
    ...
    best_results_file = f"results/{args.dataset_name}_n_clusters_{args.n_clusters}_num_K_{args.num_k}_minADE_Loss_{args.minADEloss}_best_results.csv"
